...
def _get_combined_stats(start, end=None):
    """
    Private helper — aggregate combined manual sales + online order totals.

    Returns (total_sales: Decimal, total_profit: Decimal).
    """
    sale_q = db.session.query(func.sum(Sale.total_price), func.sum(Sale.profit)).filter(
        Sale.sale_date >= start
    )
    order_q = db.session.query(func.sum(Order.total_amount)).filter(
        Order.created_at >= start, Order.order_status != 'Cancelled'
    )
    profit_q = db.session.query(func.sum(OrderItem.profit)).join(Order).filter(
        Order.created_at >= start, Order.order_status != 'Cancelled'
    )
...
def get_monthly_comparison():
    ...
    curr_s, curr_p = _get_combined_stats(current_month_start)
    last_s, last_p = _get_combined_stats(last_month_start, last_month_end)
...
def get_yearly_comparison():
    ...
    curr_s, curr_p = _get_combined_stats(current_year_start)
    last_s, last_p = _get_combined_stats(last_year_start, last_year_end)
...
def get_chart_data(days=30):
    """Get sales data for charts (Combined Manual + Online)"""
    start_date = datetime.now(timezone.utc) - timedelta(days=days)
    
    # 1. Manual Sales grouped by date
    ...
    # 2. Online Orders grouped by date (SQL aggregation instead of iteration)
    online_orders = db.session.query(
        func.date(Order.created_at).label('date'),
        func.sum(Order.total_amount).label('sales'),
        func.sum(OrderItem.profit).label('profit')
    ).join(OrderItem).filter(
        Order.created_at >= start_date, Order.order_status != 'Cancelled'
    ).group_by(func.date(Order.created_at)).all()

    sales_data_dict = {}
    for record in manual_sales:
        date_str = record.date if isinstance(record.date, str) else record.date.strftime('%Y-%m-%d')
        sales_data_dict[date_str] = {'sales': Decimal(str(record.sales or 0)), 'profit': Decimal(str(record.profit or 0))}
        
    order_data = {}
    for record in online_orders:
        date_str = record.date if isinstance(record.date, str) else record.date.strftime('%Y-%m-%d')
        order_data[date_str] = {'sales': Decimal(str(record.sales or 0)), 'profit': Decimal(str(record.profit or 0))}
...
def get_stock_stats():
    ...
    # Use joinedload to avoid N+1 queries when accessing product.category_rel.name
    products = Product.query.options(joinedload(Product.category_rel)).all()
