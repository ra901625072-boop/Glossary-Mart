...
from models import Order, Product, Sale, db


def get_sales_stats(days=1):
    """
    Get sales and profit statistics for a given number of days
    
    Args:
        days (int): Number of days to look back
        
    Returns:
        dict: Dictionary containing total_sales, total_profit, and transaction_count
    """
    ...
    return {
        ...
        'total_profit': Decimal(str(stats.total_profit)),
...
def get_monthly_comparison():
    """
    Compare current month vs last month sales and profit
    
    Returns:
        dict: Dictionary containing current and last month statistics
    """
    ...
    ).filter(Sale.sale_date >= current_month_start).first()
    ...
    from decimal import Decimal
    current_sales = Decimal(str(current_stats.total_sales or 0))
    current_profit = Decimal(str(current_stats.total_profit or 0))
    last_sales = Decimal(str(last_stats.total_sales or 0))
    last_profit = Decimal(str(last_stats.total_profit or 0))
    
    # Calculate percentage changes
    sales_change = ((current_sales - last_sales) / last_sales * Decimal('100')) if last_sales > 0 else Decimal('0')
    profit_change = ((current_profit - last_profit) / last_profit * Decimal('100')) if last_profit > 0 else Decimal('0')
...
def get_yearly_comparison():
    """
    Compare current year vs last year sales and profit
    
    Returns:
        dict: Dictionary containing current and last year statistics
    """
    now = datetime.now(timezone.utc)
    ...
    current_stats = db.session.query(
        func.sum(Sale.total_price).label('total_sales'),
        func.sum(Sale.profit).label('total_profit')
    ).filter(Sale.sale_date >= current_year_start).first()
